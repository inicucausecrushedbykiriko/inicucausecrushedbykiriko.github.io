...
async function init() {
  ...
  camera._mode = "orthographic"; // we add a camera._mode field => "orthographic" | "projective" | "fishball"
  ...
  let helpText = new StandardTextObject(
    "Quest6 Raytracing:\n" +
    "T - Cycle camera mode (Ortho/Proj/Fish)\n" +
  ...
  function updateHelpText() {
    ...
    // For the camera label
    let modeName = camera._mode;
    helpText.updateText(
      "Quest6 Raytracing:\n" +
      "T - Cycle camera mode (Ortho/Proj/Fish)\n" +
      "W/S/A/D/R/F - Move Camera\n" +
  ...
  window.addEventListener('keydown', (e) => {
    if (e.key === 'T' || e.key === 't') {
      // Cycle camera mode
      if (camera._mode === "orthographic") {
        camera._mode = "projective";
      } else if (camera._mode === "projective") {
        camera._mode = "fishball";
      } else {
        camera._mode = "orthographic";
      }
      updateHelpText();
    }
    ...
    // Switching objects
    if (e.key === ' ') {
      currentObject = (currentObject + 1) % 5;
      updateHelpText();
    }
    ...
    function moveSelected(dx, dy, dz) {
      if (currentObject === 0) {
        ...
        ellipPose[0] += dx; ellipPose[1] += dy; ellipPose[2] += dz;
        tracerObj.updateEllipsoidPose();
      }
    }
    ...
    function rotateSelected(rx, ry, rz) {
      if (currentObject === 0) {
        ...
        ellipPose[4] += rx; ellipPose[5] += ry; ellipPose[6] += rz;
        tracerObj.updateEllipsoidPose();
      }
    }
